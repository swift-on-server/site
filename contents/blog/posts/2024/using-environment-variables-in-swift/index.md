...
For example, an app could take advantage of the `LOG_LEVEL=trace` environment variable to set the log level using the Logging library ([Logging for server-side Swift apps](/logging-for-server-side-swift-apps/)) in Swift. By using an env variable developers can get more detailed logs for debugging purposes and less verbose logs for production without changing the source code of the application itself.
...
![Edit Scheme in Xcode](./assets/edit-scheme-in-xcode.png)
...
![Set env in Xcode](./assets/set-env-in-xcode.png)
...
It is possible to develop Swift projects with VSCode (see [Developing with Swift in Visual Studio Code](/developing-with-swift-in-visual-studio-code/)) using the [official Swift extension](https://www.swift.org/blog/vscode-extension/). 
...
![Set env in VSCode](./assets/set-env-in-vscode.png)
