...
@SensorActor 
final class PhoneMotionSensor: AsyncSequence {
...
Starting from Swift 6, you can specify a "task executor" to run tasks on. This is described in [SE-0417](https://github.com/apple/swift-evolution/blob/main/proposals/0417-task-executor-preference.md). This is especially useful for server-side Swift, where code can run within a SwiftNIO event loop ([SwiftNIO Fundamentals](/using-swiftnio-fundamentals/).
...
mutating func next(
    isolation actor: isolated (any Actor)? = #isolation
) async throws(MyCustomIteratorError) -> Element?
```
